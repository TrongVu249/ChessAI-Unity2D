namespace Chess {
	...
	public class Board {
		...
		//Chứa lịch sử của game state
		public uint currentGameState;

		public int plyCount; // Total plies played in game (số move đã thực hiện)
		public int fiftyMoveCounter; // Num ply since last pawn move or capture

		public ulong ZobristKey;
		/// List of zobrist keys, represents the current position on the chessboard 
		//is used for position evaluation and checking for repetitions
		public Stack<ulong> RepetitionPositionHistory;
		//Stores the Zobrist keys for previous positions. This is used to detect 
		//repetitions and potential draws
		...
		// The inSearch parameter controls whether this move should be recorded 
		//in the game history (for detecting three-fold repetition)
		public void MakeMove (Move move, bool inSearch = false) {
			uint oldEnPassantFile = (currentGameState >> 4) & 15;
			//extracted using bitwise operations
			...
			//update ZobristKey và bỏ quân khỏi piecelist

			// Move pieces in piece lists
			//If the piece being moved is a king, it updates the king's position and 
			//modifies the newCastleState based on whether castling is still possible. 
			//If the piece isn't a king, it updates the piece's position in its list.
